...
//written to insert data in database
if(isset($_POST['add_emp'])) {
  ...
  if(strlen($pass) < 4) {
    $_SESSION['error']="Password must have four characters";
    header("location:add_emp.php");
  }
  else{
    //written to check if userdata is already exist in database or not
    ...
    else
    {
      ...
      $query=mysqli_query($con,$sql);
      if($query) {
        $_SESSION['success']="<small class='text-sccess'>Data inserted Successfully</small>";
        header("location:add_emp.php");
      }
    }
